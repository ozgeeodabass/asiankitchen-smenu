...
//DOM elements: filter buttons and menu
...
let listAllFoods = () => {
  let allFoods ="";
  menu.map(item=> {allFoods += createFoodCard(item)});
...
//to list korean foods
let listKoreanFoods = () => {
  let koreanFoods="";
  menu.map(item=>{
    if(item.category==="Korea"){
      koreanFoods+= createFoodCard(item);
    }
  });
...
//to list japanese foods
let listJapaneseFoods = () => {
  let japaneseFoods="";
  menu.map(item=>{
    if(item.category==="Japan"){
      japaneseFoods+= createFoodCard(item);
    }
  });
...
//to list chinese foods
let listChineseFoods = () => {
  let chineseFoods="";
  menu.map(item=>{
    if(item.category==="China"){
      chineseFoods+= createFoodCard(item);
    }
  });
